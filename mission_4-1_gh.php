...
. " ("
. "id INT AUTO_INCREMENT NOT NULL PRIMARY KEY,"		//投稿番号INT...そのフィールドには整数値のみ入力可能
. "namae TEXT,"		//char...格納可能領域が決まっている。
. "komento TEXT,"		//TEXT...テキストのみ入力可能
. "date TEXT,"
. "pass TEXT"
.");";
$stmt1 = $pdo->query($sql1);		//query() は、指定したSQL文をデータベースに対して発行し、返された結果セット を PDOStatement オブジェクトとして返す。->は左辺から右辺を取り出す演算子.2つのカラムを持ったテーブル「keijiban」が作成される.
